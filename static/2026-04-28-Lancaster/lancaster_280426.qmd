...
::: {.incremental}
- Asymmetric tone sandhi: left- and right-dominant.
- Tonal neutralization only in the final non-dominant syllables.
- More carryover tonal coarticulation in non-dominant syllables.
- Changsha sandhi may reflect metrical prominence at the word level.
:::
...
- Yangping and Rusheng merged completely in Southwestern Mandarin.
- The falling of T42 is disappearing in young speakers, and the tone is becoming more like a mid-high level tone (T44).
:::
...
- @Bao1998, @zhongLightTonesChangsha2003, @zhongStudyStressPatterns2007: 
    - Tones are simplified and leveled in the non-dominant syllables in disyllables.
        - T45 -> T5
        - {T42, T24} -> T4
        - {T13, T34} -> T3 
        - T21 -> T1
...
1. 衣服 /**i~z~^13^**.fv̩^24-4^/ 'clothes' (Noun)
2. 洗衣服 /ɕi~z~^42-4^.**i~z~^13^**.fv̩^24-4^/ 'to wash clothes' (V-Obj)
3. 洗衣机 /**ɕi~z~^42^**.i~z~^13-3^.t͡ɕi~z~^34-3^/ 'washing machine' (Noun)
...
- The final syllable is always dominant in person names, **regardless** of the syllable count.
:::
...
3. 洪明齐 /xɤn^13-3^.min^13-3^.**t͡ɕi~z~^13-3^**/ 'Hong Mingqi' (Person name)
...
- Stress and tone can cooccur in the same language [@Hyman2006]. 
...
::: {.incremental}
- In previous research on the phonetics of tone-stress interaction, duration was found to be the most reliable acoustic correlate of stress.
...
- Duration, tonal production, and tonal coarticulation patterns consistent with the metrical prominence of the syllables?

- Stressed syllables,
    - **longer** duration, 
    - **more distinct** tonal production,
...
- How does tone sandhi affect tonal production and coarticulation?
- Previous research suggests tonal simplification through merging and leveling. Can we observe it in phonetics?
- How much phonetic evidence can we find to reinterpret the tone sandhi in Changsha Xiang as tone-stress interaction?
:::
...
- For common noun trials, 4 sets of disyllabic words were created as the target words, with each set containing 36 words for each of the tonal combinations (144 in total). For person-name trials, 2 distinct sets of disyllabic person names were used (72 in total).
...
- 7 of the 16 participants completed both experiments.

## Recording procedure

- Recording takes place in participants' homes using a Zoom H4n recorder and a Shure SM35 headset condenser microphone.
...
- F0 was extracted from 10 equidistant points in the rhyme of each syllable, and normalized using z-scores for each speaker.
...
- Generalized additive mixed-effects models (GAMM) were used to analyze the effects of tone sandhi on tonal production and coarticulation.
...
To investigate tonal coarticulation, I encoded T45 and T21 as H and L tones in the initial syllable, and T21
...
## Findings on tonal production

Comparing s1 to s2:

::: {.incremental}
- In dominant syllables,
    - T21 and T42 exhibit a slightly **smaller falling**.
    - T13, T24, T34, and T45 show a **lower** F0 minimum.
    - T24 and T45 also show a **higher** F0 maximum.
- In non-dominant syllables, 
    - T21 and T42 show a slightly **smaller falling**.
    - T13, T24, and T45 show **more** F0 excursion. 
    - T34 is lowered throughout the syllable.
:::
...
Comparing dominant to non-dominant syllables:

::: {.incremental}
- In s1,
    - T21 and T13 are entirely lower. T13 dips more in the middle.
    - T24, T34, and T45 show a **lower** F0 minimum and a **higher** F0 maximum.
    - T42 unaffected.
- In s2, 
    - T21 reaches a **lower** F0 minumum.
    - T13, T24, T34 and T45 show **rising** contour. 
:::
...
::: {.incremental}
- Anticipatory coarticulation only occurs for T24 and T34.
- Carryover coarticulation occurs for T21, T24, T42, and T45 tones, but is more extensive in non-dominant syllables.
:::

## LDA results on tonal simplification
...
- In final non-dominant syllables, the prediction accuracy of T13 and T24 becomes very low, suggesting tonal merging.
- Initial non-dominant syllables behaves largely as dominant syllables, although the prediction accuracy is lower.
:::
...
- Dominant syllables are longer than non-dominant syllables, consistent with the metrical prominence of the dominant syllables.

## How does tone sandhi affect tonal production?

- Contour tones retain their contour not only in dominant syllables, but also in initial non-dominant syllables.
- Final non-dominant syllables show significantly less F0 excuresion.
...
- Only carryover effect is more extensively influenced by tone sandhi.
...
- Tonal simplification (paradigmatic neutralization, see [@zhangDirectionalAsymmetryChinese2007]) is only found in the final non-dominant syllables, while those in initial non-dominant syllables only undergo tonal undershoot, with less F0 excursions than in dominant syllalbes.
...
- Shanghai Wu **right-dominant** tone sandhi: a pattern called *paradigmatic neutralization* [@zhangDirectionalAsymmetryChinese2007].
    ...
    - They are the four short-level tones (T1-4) in Changsha Xiang.
...
- There might have been a phrase boundary between the two syllables.
- Tones adjacent to a phrase boundary is less likely to lose their tonal identity.
